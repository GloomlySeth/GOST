...
function gost_preprocess_views_view(&$vars){
  if($vars['view']->name === 'requirements_list'){
    $vars['rows']['select']['action::views_bulk_operations_delete_item']['#value'] = t('Remove...');
...
function gost_preprocess_views_view_table(&$vars) {
  if($vars['view']->name === 'requirements_list'){
    ...
    foreach($vars['rows'] as $key => $value){
      $value['nothing'] == $user->uid ? $vars['rows'][$key]['nothing'] = t('My') : $vars['rows'][$key]['nothing'] = t('Standard');
    }
  }
}
